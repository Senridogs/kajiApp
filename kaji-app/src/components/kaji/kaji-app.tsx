...
export function KajiApp() {
  ...
  const renderTimelineRecords = (timelineGroups: TimelineRecordGroup[], emptyMessage: string, showReactions = false) => {
      ...
      <div key={`record-group-${group.dateKey}`} className="space-y-2">
        ...
        <div className="space-y-2">
          {group.items.map((record) => {
            ...
            const myReaction = showReactions ? (record.reactions ?? []).find((reaction) => reaction.userId === sessionUser.id) : undefined;
            const reactionCounts = showReactions
              ...
              : {};
            const visibleReactions = showReactions ? REACTION_CHOICES.filter((emoji) => (reactionCounts[emoji] ?? 0) > 0) : [];
            return (
      ...
      return (
        <div className="space-y-4" style={{ paddingTop: getTabHeaderHeight("records") }}>
          <div className="space-y-5" style={getPullAnimatedContentStyle(tab)}>
            {renderInlinePullRefreshHint(tab)}
            <div data-tap-priority-zone="true" className="py-1">
              <button
                type="button"
                onClick={() => {
            ...
            {renderTimelineRecords(groupedTimelineRecords, "家事を完了するとここにタイムライン表示されます。", true)}
          </div>
        </div>
      );
    }
